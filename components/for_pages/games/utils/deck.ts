...
export default class CardsDeck {
 ...
 static getRandomCard(): number{
   return getRandom(1, Object.keys(CardsDeck.deck).length -1)
 }
  static getImagePath(cardIndex: number): string | null{
   const card = this.deck[cardIndex]
   if(!card){
     return null
   }
   return `/img/Games/cards/${card.type}/${card.value}.svg`
  }
