...
import {BonusDepositShowMode, ModalType, ProfileModalType} from 'types/enums'
    ...
    <RemoveScroll enabled={!!context.modal}>
      {!isServer && (
        ...
        <div aria-hidden="true">
          <ProfileModalNew key={1} size={'small'} isOpen={context.modal === ModalType.login} {...commonSettings}>
            {context.modal === ModalType.login && <ModalLogin/>}
          </ProfileModalNew>
          <ProfileModalNew key={2}  size={'small'} isOpen={context.modal === ModalType.passwordRecovery} {...commonSettings}>
            {context.modal === ModalType.passwordRecovery && <ModalPasswordRecovery/>}
          </ProfileModalNew>
          <ProfileModalNew key={3}  size={'small'} isOpen={context.modal === ModalType.passwordReset} {...commonSettings}>
            {context.modal === ModalType.passwordReset && <ModalPasswordReset/>}
          </ProfileModalNew>
          <ProfileModalNew key={4}   size={'small'} isOpen={context.modal === ModalType.registration} {...commonSettings}>
            {context.modal === ModalType.registration && <ModalRegistration/>}
          </ProfileModalNew>
          <ProfileModalNew key={5}  size={'small'} isOpen={context.modal === ModalType.registrationPhone} {...commonSettings}>
            <ModalRegistrationPhone/>
          </ProfileModalNew>
          <ProfileModalNew key={6}  size={'small'} isOpen={context.modal === ModalType.registrationSuccess} {...commonSettings}>
            {context.modal === ModalType.registrationSuccess && <ModalRegistrationSuccess/>}
          </ProfileModalNew>
          <ProfileModalNew size='large' key={7}
          ...
          <Modal key={13} isOpen={context.modal === ModalType.bonus} 
          {...commonSettings} onRequestClose={() => {
            context.setBonusShowMode(BonusDepositShowMode.Spoiler)
            context.hideModal()
          }} noBorder size='bonus'>
  
            {context.modal === ModalType.bonus &&   <BonusModal {...commonSettings}/>}
          </Modal>
         
          <ProfileModalNew
            size='small'
            key={15}  isOpen={context.modal === ProfileModalType.exchange}>
            {context.modal === ProfileModalType.exchange && <Exchange/>}
          </ProfileModalNew>
          <ProfileModalNew size={'large'} key={16}
                           isOpen={context.modal === ProfileModalType.favorite} {...commonSettings} >
            {context.modal === ProfileModalType.favorite && <Favorite/>}
          </ProfileModalNew>
          <ProfileModalNew size={'large'} key={17}
                           isOpen={context.modal === ProfileModalType.betsHistory} {...commonSettings} >
            <BetsHistory/>
          </ProfileModalNew>
          <ProfileModalNew
            size='small'
            key={18}  isOpen={context.modal === ProfileModalType.buyCrypto} {...commonSettings}
            >
            {context.modal === ProfileModalType.buyCrypto && <BuyCrypto/>}
          </ProfileModalNew>
          <ProfileModalNew
            size='small'
            key={19} isOpen={context.modal === ProfileModalType.FA} {...commonSettings}
        >
          ...
          <ProfileModalNew
            size='small'
            key={20}  isOpen={context.modal === ModalType.profileBurger} {...commonSettings}
          >
            <ProfileBurger onRequestClose={() => context.hideModal()}/>
          </ProfileModalNew>
          <ProfileModalNew
            size='small'
            key={21}  isOpen={context.modal === ProfileModalType.oldPhoneConfirm} {...commonSettings}
          >
            {context.modal === ProfileModalType.oldPhoneConfirm && <ModalConfirmOldPhone/>}
          </ProfileModalNew>
          <ProfileModalNew
            size='small'
            key={22}  isOpen={context.modal === ProfileModalType.newPhoneConfirm} {...commonSettings}
          >
            {context.modal === ProfileModalType.newPhoneConfirm && <ModalConfirmNewPhone/>}
          </ProfileModalNew>
          <ProfileModalNew
            size='small'
            key={23}  isOpen={context.modal === ModalType.faLogin} {...commonSettings}
          >
            {context.modal === ModalType.faLogin && <FALogin/>}
          </ProfileModalNew>
          <ProfileModalNew key={24} size={'small'} isOpen={context.modal === ModalType.mobileApp} {...commonSettings}>
